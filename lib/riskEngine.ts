import type { RiskLevel, UploadRecord } from "@/types";
...
export function categorizeTool(toolName: string) {
  ...
  if (categoryMap[compact]) return categoryMap[compact];
  if (aiSignals.some((signal) => compact.includes(signal))) return "AI";
  return "Unknown";
}
...
export function scoreRisk(input: { toolName: string; domain: string; permissions: string[]; category?: string }) {
  const domain = normalizeDomain(input.domain);
  const category = input.category ?? categorizeTool(input.toolName);
  const known = knownSaaS.has(input.toolName.toLowerCase());
...
export function scoreRecord(record: UploadRecord) {
  ...
  return { permissions, ...scoreRisk({ toolName: record.tool_name, domain: record.domain, permissions }) };
}
...
export async function generateRiskExplanation(input: {
  ...
  if (!process.env.GEMINI_API_KEY) return deterministicExplanation(input);
  ...
  try {
          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `Write a concise SaaS security risk explanation for this JSON. Include why it is risky, likely data exposure, and an admin recommendation: ${JSON.stringify(input)}`
                }
              ]
            }
          ],
    ...
    if (!response.ok) return deterministicExplanation(input);
    const data = await response.json();
    return data.candidates?.[0]?.content?.parts?.[0]?.text ?? deterministicExplanation(input);
  } catch {
